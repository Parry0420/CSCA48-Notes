 ...
 *
 * Starter code:  Angela Zavaleta & Mustafa Quraish
 */
               ...
               float max_price, Furniture *results[])
{
  ...
  int matches=0;
  for(int i=0;i<catalog_size;i++){
    if(catalog[i].price <= max_price && catalog[i].price >= min_price){
      results[matches] =  &catalog[i];
      matches++;
    }
  }
...
char *biggest_ripoff_name(Furniture catalog[], int catalog_size)
{
  ...
  float biggest_ripoff = catalog[0].price /(catalog[0].dimensions.depth * 
  catalog[0].dimensions.width * catalog[0].dimensions.height);
  Furniture *biggest = catalog;
  for(int i=1;i<catalog_size;i++){
    if(catalog[i].price / (catalog[i].dimensions.depth * 
      catalog[i].dimensions.width * catalog[i].dimensions.height) 
      > biggest_ripoff){
      biggest_ripoff = catalog[i].price / (catalog[i].dimensions.depth * 
                    catalog[i].dimensions.width * catalog[i].dimensions.height);
      biggest = &catalog[i];            
  }
  }
